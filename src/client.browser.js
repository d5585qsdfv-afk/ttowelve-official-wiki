...
function populateJobFilters(){const classes=entries.filter(entry=>entry.category==='jobs').map(jobMetadata).map(meta=>meta.characterClass),counts=new Map();classes.forEach(value=>counts.set(value,(counts.get(value)||0)+1));const select=$('#jobClass'),previous=select.value;select.innerHTML='<option value="">すべてのクラス</option>'+vundClasses.map(value=>`<option value="${esc(value)}">${esc(value)} (${counts.get(value)||0})</option>`).join('')+`<option value="クラス未確定">クラス未確定 (${counts.get('クラス未確定')||0})</option>`;select.value=vundClasses.includes(previous)||previous==='クラス未確定'?previous:''}
function filtered(){
