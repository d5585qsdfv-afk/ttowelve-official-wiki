...
function jobMetadata(entry){const subtitle=entry.subtitle||'';return {characterClass:subtitle.split('｜')[1]||entry.characterClass||'クラス未設定'}}
...
function populateJobFilters(){const classes=entries.filter(entry=>entry.category==='jobs').map(jobMetadata).map(meta=>meta.characterClass);fillFilterSelect('#jobClass',classes,'すべてのクラス')}
function filtered(){
...
function render(){
  ...
  $('#entryGrid').innerHTML=list.map(e=>{
    ...
    return `<button class="entry ${cardClass}" data-id="${esc(e.id)}" aria-label="${esc(e.title)}の詳細を開く" style="--accent:${accents[e.accent]||accents.lime}">${e.image?`<span class="entry-media"><img src="${esc(e.image)}" alt="${esc(e.imageAlt||e.title)}" loading="lazy"></span>`:''}<span class="entry-overline"><span class="entry-kind">${esc(labels[e.category]||e.category)}</span>${meta?`<span class="enemy-class-badge">◇ ${esc(meta.classification)}</span>`:e.status&&e.status!=='実装済み'?`<span class="status-badge">${esc(e.status)}</span>`:weapon?`<span class="weapon-type-badge">${esc(weapon.tacticalType)}</span>`:job?`<span class="weapon-type-badge">${esc(job.characterClass)}</span>`:''}</span><h2>${esc(e.title)}</h2>${meta?`<span class="subtitle">${esc(meta.number)}</span>`:!weapon?`<span class="subtitle">${esc(e.subtitle)}</span>`:''}${quickFacts}${job?`<span class="entry-facts">クラス｜${esc(job.characterClass)}</span>`:''}${card?`<span class="entry-facts">Lv.${card.levels.join(' · ')}｜${esc(card.role)}｜${esc(card.channel==='セット発動'?'セット':'CH '+normalizeDigits(card.channel))}</span>`:''}${medicine?`<span class="entry-facts">${esc(medicine.timing)}｜調合薬</span>`:''}<p class="summary">${esc(e.summary)}</p><footer><span class="tags">${(e.tags||[]).filter(tag=>!weapon||![weapon.tacticalType,weapon.type].includes(tag)).slice(0,3).map(t=>`<span class="tag">${esc(t)}</span>`).join('')}</span><span class="entry-open">詳細 <span aria-hidden="true">↗</span></span></footer></button>`;
  }).join('');
