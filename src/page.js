...
// The Worker/API is the authoritative data source. Keep only a tiny bootstrap
// value in the HTML so the page does not ship the complete catalog twice.
const initialData = '[]';
...
<body>
  ...
  <dialog class="account-dialog" id="accountDialog">
    <article class="account-card">
      ...
      <section id="signedInPanel" class="hidden"><h3>お気に入りを自動同期</h3><p>図鑑・タグのお気に入りと比較用の固定項目は、ログイン中のアカウントに自動保存されます。別端末で同じアカウントにログインすると自動で反映されます。</p><p id="cloudStatus" class="form-message" role="status">ログインするとクラウドとの自動同期が始まります。</p><div class="account-actions"><button type="button" class="secondary" data-action="cloud-load">今すぐ同期</button><button type="button" class="primary" data-action="cloud-save" disabled>今すぐ保存</button><button type="button" class="secondary" data-action="cloud-restore" disabled>クラウドから復元</button></div><div class="account-actions"><button type="button" class="secondary" data-action="sign-out">ログアウト</button></div></section><section id="proposalPanel" class="hidden">
