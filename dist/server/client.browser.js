...
function populateFilters(){
  ...
  populateCardFilters();populateMedicineFilters();
}
...
function weaponMetadata(entry){
  ...
  return {rank:parts[0]||'—',type:parts[1]||'未分類',classification:field('基本分類')||parts.slice(2).join('、')||'未設定',damage:compactDamage(field('基礎ダメージ')),multiplier:(body.match(/属性倍率\s*([０-９0-9.]+倍)/)||[])[1]||'—'};
}
...
function populateCardFilters(){const cards=entries.filter(entry=>entry.category==='cards'),metas=cards.map(cardMetadata);fillFilterSelect('#cardLevel',[...new Set(metas.flatMap(meta=>meta.levels))].sort((a,b)=>Number(a)-Number(b)),'すべてのレベル',value=>'Lv.'+value);const roleOrder=['Attack','Support','Health','Counter','Character'];fillFilterSelect('#cardRole',roleOrder.filter(role=>metas.some(meta=>meta.role===role)),'すべてのロール');fillFilterSelect('#cardChannel',[...new Set(metas.map(meta=>meta.channel))].sort((a,b)=>normalizeDigits(a).localeCompare(normalizeDigits(b),'ja',{numeric:true})),'すべてのチャンネル',value=>value==='セット発動'?'セット発動':'CH '+normalizeDigits(value))}
function populateMedicineFilters(){const timings=[...new Set(entries.filter(entry=>entry.category==='medicines').map(medicineMetadata).map(meta=>meta.timing))];fillFilterSelect('#medicineTiming',timings,'すべての区分')}
function filtered(){
...
function render(){
  const list=filtered();$('#countAll').textContent=entries.length;$('#resultTitle').textContent=labels[activeCategory];$('#resultCount').textContent=list.length+'件';$('#empty').classList.toggle('hidden',list.length>0);$('#enemyFilters').classList.toggle('hidden',activeCategory!=='enemies');$('#cardFilters').classList.toggle('hidden',activeCategory!=='cards');$('#medicineFilters').classList.toggle('hidden',activeCategory!=='medicines');
  $('#entryGrid').innerHTML=list.map(e=>{
    const meta=e.category==='enemies'?enemyCardMetadata(e):null,weapon=e.category==='weapons'?weaponMetadata(e):null;
    const card=e.category==='cards'?cardMetadata(e):null,medicine=e.category==='medicines'?medicineMetadata(e):null;
    const cardClass=meta?'enemy-entry':weapon?'weapon-entry':medicine?'medicine-entry':'';
    const quickFacts=weapon?`<dl class="entry-stats weapon-stats"><div><dt>格付</dt><dd>${esc(weapon.rank)}</dd></div><div><dt>系統</dt><dd>${esc(weapon.type)}</dd></div><div><dt>基礎</dt><dd>${esc(weapon.damage)}</dd></div><div><dt>属性倍率</dt><dd>${esc(weapon.multiplier)}</dd></div></dl><p class="entry-classification"><span>戦闘特性</span>${esc(weapon.classification)}</p>`:meta?`<dl class="entry-stats enemy-stats"><div><dt>HP</dt><dd>${esc(meta.hp)}</dd></div><div><dt>章</dt><dd>${esc(meta.chapter)}</dd></div><div class="wide"><dt>出現場所</dt><dd>${esc(meta.location)}</dd></div></dl><p class="affinity" aria-label="属性相性">${esc(meta.affinity)}</p><p class="enemy-action"><span>主な行動</span>${esc(meta.action)}</p>`:'';
    return `<button class="entry ${cardClass}" data-id="${esc(e.id)}" aria-label="${esc(e.title)}の詳細を開く" style="--accent:${accents[e.accent]||accents.lime}"><span class="entry-overline"><span class="entry-kind">${esc(labels[e.category]||e.category)}</span>${meta?`<span class="enemy-class-badge">◇ ${esc(meta.classification)}</span>`:weapon?`<span class="weapon-rank-badge">${esc(weapon.rank)}</span>`:''}</span><h2>${esc(e.title)}</h2>${meta?`<span class="subtitle">${esc(meta.number)}</span>`:!weapon?`<span class="subtitle">${esc(e.subtitle)}</span>`:''}${quickFacts}${card?`<span class="entry-facts">Lv.${card.levels.join(' · ')}｜${esc(card.role)}｜${esc(card.channel==='セット発動'?'セット':'CH '+normalizeDigits(card.channel))}</span>`:''}${medicine?`<span class="entry-facts">${esc(medicine.timing)}｜調合薬</span>`:''}<p class="summary">${esc(e.summary)}</p><footer><span class="tags">${(e.tags||[]).filter(tag=>!weapon||![weapon.rank,weapon.type].includes(tag)).slice(0,3).map(t=>`<span class="tag">${esc(t)}</span>`).join('')}</span><span class="entry-open">詳細 <span aria-hidden="true">↗</span></span></footer></button>`;
  }).join('');
}
...
document.addEventListener('click',event=>{
  ...
  else if(action==='reset-database-filters'){[...cardFilterFields,...medicineFilterFields].forEach(selector=>$(selector).value='');$('#search').value='';render()}
...
filterFields.forEach(([selector])=>$(selector).addEventListener('change',render));
 [...cardFilterFields,...medicineFilterFields].forEach(selector=>$(selector).addEventListener('change',render));
