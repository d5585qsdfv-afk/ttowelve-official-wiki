...
function render(){
  ...
  $('#entryGrid').innerHTML=list.map(e=>{
    ...
    return `<button class="entry ${meta?'enemy-entry':medicine?'medicine-entry':''}" data-id="${esc(e.id)}" style="--accent:${accents[e.accent]||accents.lime}"><span class="entry-kind">${esc(labels[e.category]||e.category)}</span>${meta?`<span class="enemy-class-badge">◇ ${esc(meta.classification)}</span>`:''}<h2>${esc(e.title)}</h2><span class="subtitle">${esc(meta?[meta.chapter,meta.location].join('｜'):e.subtitle)}</span>${card?`<span class="entry-facts">Lv.${card.levels.join(' · ')}｜${esc(card.role)}｜${esc(card.channel==='セット発動'?'セット':'CH '+normalizeDigits(card.channel))}</span>`:''}${medicine?`<span class="entry-facts">${esc(medicine.timing)}｜調合薬</span>`:''}<p class="summary">${esc(e.summary)}</p><footer><span class="tags">${(e.tags||[]).slice(0,3).map(t=>`<span class="tag">${esc(t)}</span>`).join('')}</span><span class="entry-arrow">↗</span></footer></button>`;
  }).join('');
}
