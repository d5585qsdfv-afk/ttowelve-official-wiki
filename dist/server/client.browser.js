...
function togglePinnedEntry(id){const result=togglePinnedIds(pinnedEntryIds,id,MAX_PINNED_ENTRIES);if(!result.changed){pinNotice=result.reason==='limit'?`同時表示できる詳細は最大${MAX_PINNED_ENTRIES}件です。固定を外してから追加してください。`:'';render();return}pinnedEntryIds=result.ids;pinNotice='';savePinnedEntryIds();render();updateDetailControls();scheduleArchiveSync()}
...
renderAccount();void refreshAccess();if(currentSession()?.access_token)void pullArchivePreferences(true).catch(error=>archiveStatus(error.message||'クラウドと同期できませんでした。'));populateFilters();render();load();
